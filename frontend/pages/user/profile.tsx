  ...
  return (
    <>
      <div className="w-full flex absolute justify-end h-full top-0 right-0  ">
        <div
          className={`background z-10 absolute w-full h-full ${styles.background} `}
        ></div>
        ...
        <div className="contain absolute w-full h-full flex xl:justify-between">
          {openfriends || opensettings ? (
            <div className="si xl:hidden absolute lg:hidden md:hidden z-60  sm:hidden hidden">
              <Sidebar />
            </div>
          ) : (
          ...
          <div className="allinfo  z-50   w-full  xl:w-[84%] 2xl:w-[89%] h-full">
            {openfriends ? (
              <div className="popup absolute justify-center  z-50 w-full items-center h-full flex  ">
                <div
                  className="bgopaci absolute top-0 left-0 w-full h-full  min-w-full bg-opacity-95  bg-[#463573] "
                ...
                <div className="popupsettings absolute z-10 bg-white w-[90%] sm:w-[80%] md:w-[70%] lg:w-[50%] xl:w-[60%] 2xl:w-[800px]  h-[50%] rounded-[18px]">
                  <div className="closebtn w-full flex justify-end p-8">
                    <button
                      className="text-[23px]  flex justify-end text-[#000] w-[30px] "
                      onClick={() => setopenfriends(!openfriends)}
                    >
                      <img src="/profile/popup/exit.png" alt="" />
                    </button>
                  </div>
                  <div className="friend w-full flex justify-center">
                    <h1 className="text-[25px] font-bold text-[#42386f]">
                      All Friends <span>{"(14)"}</span>
                    </h1>
                  </div>
                    ...
                    {listFreinds.map((friends, index) => {
                      return (
                        <>
                          <div
                            key={index}
                            className="list  bg-[#dddae4] w-[90%] h-24 mt-3 rounded-[20px] justify-around flex items-center px-5"
                          >
                            ...
                            <div className="btns flex">
                              <div
                                className={`btnprofile flex p-5 justify-center rounded-[20px] cursor-pointer ${styles.btnprofile}`}
                              >
                                <img
                                  src="/profile/popup/user.png"
                                  className="w-[30px] "
                                  alt=""
                                />
                                <p className="px-2 text-[19px] font-bold text-[#4b3d7f]">
                                  Profile
                                </p>
                              </div>
                              <div
                                className={`btncontact mx-2 flex p-5 justify-center rounded-[20px] cursor-pointer ${styles.btnconact}`}
                              >
                                <img
                                  src="/profile/popup/chat.png"
                                  className="w-[30px]"
                                  alt=""
                                />
                                <p className="px-2 text-[19px] font-bold text-[#fff]">
                                  Contact
                                </p>
                              </div>
                            </div>
                  ...
                  <div className="menu bg-opacity-50 bg-[#644dad] w-full flex rounded-[20px] flex-col md:flex-row ">
                    <div className="firstbtn flex py-2 px-2 flex-col md:flex-row md:justify-between w-full">
                      <div className="btns flex flex-grow ">
                        <h1
                          onClick={() => setview_history(false)}
                          style={{
                            background: view_history ? "#5d48a3db" : "#705bb1",
                          }}
                          className=" flex-grow md:flex-grow-0 sm:text-[25px] md:px-4 lg:px-9 font-bold   py-4 rounded-[19px] bg-[#705bb1] text-[#cec0fb] flex justify-center  cursor-pointer"
                        >
                          Overview
                        </h1>
                        <h1
                          onClick={() => setview_history(true)}
                          style={{
                            background: view_history ? "#705bb1" : "#5d48a3db",
                          }}
                          className="sm:text-[25px] flex-grow md:flex-grow-0 font-bold md:px-4 lg:px-9  mx-1 py-4 rounded-[19px] bg-[#5d48a3db] text-[#cec0fb] flex justify-center  cursor-pointer"
                        >
                          Match History
                        </h1>
                        <h1
                          onClick={() => setopenfriends(!openfriends)}
                          className="sm:text-[25px] font-bold  mx-2 py-4 rounded-[19px] flex-grow md:flex-grow-0 bg-[#5d48a3db] md:px-4 lg:px-9 text-[#cec0fb] flex justify-center  cursor-pointer"
                        >
                          Friends
                        </h1>
                      </div>
                      <div className="btnsettings">
                        <h1
                          onClick={() => setopensettings(!opensettings)}
                          className={`text-[25px] relative grow md:flex-grow-0 font-bold  mx-3 py-4 rounded-[19px] lg:px-9 text-[#3f2d70] flex justify-center  cursor-pointer ${styles.btnsettings} `}
                        >
                          Settings
                        </h1>
                      </div>
