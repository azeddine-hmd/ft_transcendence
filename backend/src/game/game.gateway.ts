...
export class GameGateway {
	...
	async handleConnection(client: Socket, ...args: any[]) {
		try {
			...
			await this.usersSocketService.setStatus(client.user.userId, 'in game');
		} catch (exception) {
	...
	async handleDisconnect(client: Socket, ...args: any[]) {
		if (client.user) {
			try {
				await this.usersSocketService.setStatus(client.user.userId, '');
			} catch (exception) {
			}
	...
	gameEnd(@ConnectedSocket() socket: Socket, @MessageBody() _data: string) {
		...
		if (!_game || !_game[mode] || !_game[mode][index])
			return;
